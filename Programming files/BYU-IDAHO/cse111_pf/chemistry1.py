...
def atomic_mass(symbol):
  return make_periodic_table()[symbol][1]
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for symbol_quantity in symbol_quantity_list:
      ...
      molar_masses += mass * quantity
    return molar_masses
    
def main():
    table = make_periodic_table()
    moleformular = input("Enter molecular formulation : ")
    mass = float(input("Enter mass of substance (g) : "))
    molar_mass = round(compute_molar_mass(parse_formula(moleformular, table), table), 5)
    print("Molar mass :", molar_mass)
    num_of_moles = round(mass / molar_mass, 5)
    print("Number of moles :", num_of_moles)
